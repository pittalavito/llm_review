...
from db.tables import PromptVersionTable
...
class PromptRepository:
    ...
    ) -> list[PromptVersionTable]:
        statement = select(PromptVersionTable)
        if agent_role is not None:
            statement = statement.where(PromptVersionTable.agent_role == agent_role)
        if not include_inactive:
            statement = statement.where(PromptVersionTable.is_active == True)  # noqa: E712
        statement = statement.order_by(
            PromptVersionTable.agent_role, PromptVersionTable.version_label
        )
    ...
    def get_by_role_label(
        ...
        if row is None or (only_active and not row.is_active):
            raise ValueError(
                f"Prompt version not available: {agent_role}/{version_label}"
            )
        return row
    ...
    def create(
        ...
        with Session(self._engine) as session:
            ...
            if duplicate is not None:
                raise ValueError(
                    f"Prompt version already exists: {agent_role}/{version_label}"
                )
